        ...
        NavigationStack {
            Group {
                if viewModel.categories.isEmpty && viewModel.error == nil {
                    ...
                    // See LiveSplitView for why we landed on .defaultFocus
                    // instead of .prefersDefaultFocus + .focusScope.
                    // Value resolves to nil while categories load
                    // (no-op then), takes effect once they do.
