...
/// VOD category grid — spacious visual cards.
struct VODCategoryListView: View {
    ...
    private let categoryColors: [Color] = [
        .purple, .blue, .teal, .green, .orange, .pink, .red, .indigo, .mint, .cyan
    ]
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationDestination(for: Category.self) { category in
                VODGridView(category: category, viewModel: viewModel, api: api)
            }
        }
        .task {
