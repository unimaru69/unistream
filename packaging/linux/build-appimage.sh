...
bundle_lib() {
  ...
  case "$libname" in
    ...
    libstdc++.so*) return 0 ;;
    # NOTE: libmpv and FFmpeg libs ARE bundled (not excluded) — the Flutter
    # binary is linked against the CI's Ubuntu libmpv, so we must ship it.
  esac
...
# Pass 2: explicitly bundle libmpv and its transitive deps
