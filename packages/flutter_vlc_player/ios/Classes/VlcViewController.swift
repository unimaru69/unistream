...
public class VLCViewController: NSObject, FlutterPlatformView {
    ...
    init(frame: CGRect, viewId: Int64, messenger:FlutterBinaryMessenger) {
        ...
        // UniStream patch (vs upstream 7.4.3): pass libvlc-instance options
        // to MobileVLCKit's VLCMediaPlayer init so subtitle-rendering
        // controls (which are libvlc_new()-time only — silently ignored
        // when added at media level) actually take effect.
        //
        // `--freetype-rel-fontsize=20` selects the smallest preset on the
        // libvlc 3 enum (default is 16 = oversized at TV viewing distance);
        // matches what libmpv produces on macOS/Linux at sub-font-size 24.
        // See `tvos/UniStreamTV/.../VLCPlayerViewController.swift` for the
        // tvOS counterpart, which we can pass at instance level natively.
        self.vlcMediaPlayer = VLCMediaPlayer(options: [
